...
# Flow [B]: [Beneficiary_PreBooked]
## Who this person is
This person is a beneficiary of the policy holder who has pre-booked a doctor consultation or lab visit (either AHC which could be a combination of tests - both pathology and radiology or could be only pathology). Essentially, what this means is that the policy holder would have logged in to the Visit app and would have booked the appointment for the beneficiary.

## How they arrived at the kiosk (trigger)

They walk in to the clinic and walked to the kiosk with a whatsapp message which has the QR code confirmation of the appointmen.
...
- What the patient/user sees:
  a. Message saying  - Please enter mobile number for us to fetch your details or Scan scan the QR code that was provided to you against your appointment. (You can find your QR in your Visit App)
...
- What system does in background:
     ...
     After the OTP is verified, calls and API to fetch patient details and then another API call to fetch orders against the patient for today.#
...
- What the system does in background:
    Calls the service which scans the QR and then calls the API to fetch the patient details then another API to fetch orders against the patient for today. 

### S3: [List of appointments/orders]
- What the user sees:
    a. Welcome {beneficiary_name}, where {beneficiary_name} is the full name of the patient fetched by calling the API.
