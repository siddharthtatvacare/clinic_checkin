...
- What the user sees:
    ...
    c. The list of the appointments/orders (Dr appointments/ Lab or AHC - show relevant icons, appointment type, doctor/test name, and time) for today against this patient with the CTA for check-in. Do not show location (cabin/floor) on this screen — location is communicated on S4 after check-in. Note that the check-in is not at order level but overall into the clinic.
- What action the user takes:
    Clicks on Check-in

### S4: [Queueing and Checkin]
- What the user sees:
 A message telling the user that you have successfully checked in and display the token number of the user. 
 Also communicate to the patient that they will receive whatsapp updates and also updates in the app which helps guide them through their journey within the clinic.
