...
**Architecture:** Four layers: (1) `demo-shell/` wraps the kiosk for demo and is deleted in production; (2) `kiosk/` is the real product; (3) `kiosk/services/` is the API + telemetry layer — mock in demo, real in production, swapped via `VITE_USE_REAL_API`; (4) `kiosk/session/`, `kiosk/errors/`, `kiosk/telemetry/`, `kiosk/layout/` provide production infrastructure (idle management, error boundaries, observability, shared patterns). S1 Welcome lives at the `KioskApp` level (shared entry point for all 5 flows).

**Tech Stack:** Vite 6 + React 18 + TypeScript, Tailwind CSS v4 (`@tailwindcss/vite`), Framer Motion, Lucide React, `@zxing/library` (QR camera reading), Vitest + React Testing Library, vite-plugin-pwa
...
| Theme enforcement | Tailwind v4 `@theme` → semantic classes | `bg-brand` not `bg-[#714FFF]` — single source of truth |
...
├── tsconfig.json
├── vitest.config.ts
├── .env.development                    ← VITE_USE_REAL_API=false, device vars
├── .env.production                     ← VITE_USE_REAL_API=true
│
├── public/
│   └── visit-logo.svg
│
...
**Files:**
- Create: `package.json`, `vite.config.ts`, `tsconfig.json`, `vitest.config.ts`, `index.html`, `.env.development`, `.env.production`, `tests/setup.ts`

- [ ] **Step 1: Scaffold the Vite + React + TS project**

Run from `check-in/` (parent directory):
```bash
npm create vite@latest checkin_proto -- --template react-ts
cd checkin_proto
```
...
VITE_DEVICE_ID=dev-kiosk-01
VITE_CLINIC_ID=dev-clinic-001
VITE_CLINIC_NAME=Visit Health - Dev

# .env.production
VITE_USE_REAL_API=true
VITE_API_BASE_URL=https://api.visithealth.in
VITE_DEVICE_ID=                   # Set during kiosk provisioning
VITE_CLINIC_ID=                   # Set during kiosk provisioning
VITE_CLINIC_NAME=                 # Set during kiosk provisioning
```
...
```bash
git init && git add . && git commit -m "feat: scaffold Vite+React+TS, Tailwind v4, Vitest, PWA, env config"
```
...
- [ ] **Step 1: Create src/index.css with @theme directive**
...
@theme {
  --color-brand:          #714FFF;
  --color-brand-dark:     #593FC9;
  --color-accent:         #FF9273;
  --color-text-primary:   #1F1E1E;
  --color-text-secondary: #686670;
  --color-bg-surface:     #F5F3FF;
  --color-bg-card:        #FFFFFF;
  --color-success-bg:     #E6F9F0;
  --color-success-text:   #2d7a50;
  --color-error-bg:       #FFF0EB;
  --color-border-light:   #ede9ff;
}
...
This generates Tailwind utilities: `bg-brand`, `text-brand-dark`, `bg-accent`, `text-text-primary`, `text-text-secondary`, `bg-bg-surface`, `bg-bg-card`, `bg-success-bg`, `text-success-text`, `bg-error-bg`, `border-border-light`.
...
// For className, use Tailwind @theme utilities: bg-brand, text-accent, etc.
...
git commit -m "feat: @theme tokens, CSS custom properties, all domain + infra types"
```
...
```bash
npx vitest run tests/hooks/useIdleTimer.test.tsx
```
...
```bash
npx vitest run tests/hooks/useApiCall.test.tsx
```
...
export function loadDeviceConfig(): DeviceConfig {
  ...
  _config = {
    deviceId:   import.meta.env.VITE_DEVICE_ID   || 'dev-kiosk',
    clinicId:   import.meta.env.VITE_CLINIC_ID    || 'dev-clinic',
    clinicName: import.meta.env.VITE_CLINIC_NAME  || 'Dev Clinic',
  }
  return _config
}
...
export const realTelemetry: TelemetryService = {
  ...
  async flush() {
    ...
    try {
      ...
      await fetch(`${import.meta.env.VITE_API_BASE_URL || ''}/kiosk/telemetry`, {
        method: 'POST',
...
export const telemetry = import.meta.env.VITE_USE_REAL_API === 'true'
  ? realTelemetry
  : mockTelemetry
```
...
> ```
> VITE_VISIT_API_URL=https://api.visithealth.in
> VITE_TP_API_URL=https://api.tatvapractice.in
> ```
...
const VISIT_BASE = import.meta.env.VITE_VISIT_API_URL ?? ''
const TP_BASE    = import.meta.env.VITE_TP_API_URL    ?? ''
...
export const api = import.meta.env.VITE_USE_REAL_API === 'true' ? realAPI : mockAPI
```
...
- [ ] **Step 3: Implement Button (using @theme tokens)**
...
- [ ] **Step 5: Create Card and Badge (using @theme tokens)**
...
git commit -m "feat: Button (TDD), Card, Badge — all using @theme tokens"
```
...
git commit -m "feat: NumericKeypad + OTPInput with tests — using @theme tokens"
```
...
git commit -m "feat: QueueTokenCard (TDD), ProgressBar, StepHeader — @theme tokens"
```
...
npm run preview
```
Open `http://localhost:4173` — verify iPad frame, Flow A full navigation.

- [ ] **Step 5: Audit — no inline hex in kiosk/ components**

```bash
grep -rn '#714FFF\|#FF9273\|#686670\|#1F1E1E\|#F5F3FF\|#ede9ff' src/kiosk/ --include='*.tsx' | grep -v 'theme.ts' | grep -v '.test.'
```
Expected: zero matches (all colors should use `@theme` utilities or `theme.ts` imports).
...
- [ ] `npm run dev` → iPad frame visible, Visit logo, 5-option home screen
...
- [ ] `VITE_USE_REAL_API=true npm run build` → builds cleanly
